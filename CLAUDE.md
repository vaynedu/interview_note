...
### 4.5 答题模板 D：多概念对比（多维度对比 + 协作时序 + 缺一不可）⭐

> 用于讲**多个相关概念的对比 / 选型 / 协作**的场景。
>
> 适用：MySQL 三种日志（redo/undo/binlog）、Redis 持久化（RDB/AOF）、分布式事务（2PC/TCC/Saga）、
> Kafka vs RocketMQ vs RabbitMQ、select/poll/epoll、Cache-Aside vs Write-Through、ZK vs etcd vs Redis 锁……
>
> **目标**：让读者一眼看清"差异 + 协作 + 取舍"，不再死记硬背。
...
3. 协作时序 / 流程图（Mermaid）
   - 多概念在一次完整流程中的协作关系
   - 时序图（sequenceDiagram）或流程图（flowchart）
   - 标注每个概念出场的时机和作用
...
| 主题 | 文件 |
| --- | --- |
| MySQL 三类日志（redo/undo/binlog）| [03-mysql/04-log.md 第一章](03-mysql/04-log.md) ⭐ |
| 三方分布式锁（Redis/ZK/etcd）| [06-distributed/04-lock.md 七横向对比](06-distributed/04-lock.md) |
| 分布式事务 5 种方案 | [06-distributed/03-transaction.md 选型决策](06-distributed/03-transaction.md) |
| MQ 选型（Kafka/RocketMQ/RabbitMQ/Pulsar）| [05-message-queue/00-mq-map.md 选型对比表](05-message-queue/00-mq-map.md) |

**何时用 D 模板（vs B 5 段式）**：

| 场景 | 用 B 还是 D |
| --- | --- |
| 讲单个机制（B+ 树 / Raft / MVCC）| **B（5 段式）** |
| 讲多个并列机制的差异（redo vs undo vs binlog）| **D（多概念对比）** |
| 讲选型（用什么）| **D** |
| 讲单点的演进（select → poll → epoll）| **B**（演进是 5 段式的一部分）|
| 同时有"对比"和"深度"需求 | 先 D 再嵌入多个 B 子节 |
...
| **D. 多概念对比** ⭐ | **多概念对比 / 选型** | **透彻理解差异 + 协作** | **03-mysql/04-log** |

**写新文件时按"主要用途"选模板**：
- 写"X 是什么 / 怎么实现"→ B（5 段式）
- 写"我做了 X 项目"→ A（STAR-L）
- 写"高频问 X 怎么答"→ C（四段式）
- 写"X vs Y / 怎么选" → **D（多概念对比）**

**可以组合**：
- 一个文档可以用多种模板（如某文件目录用 B、第一章用 D、末尾速答用 C）
- D 模板里嵌入 B 模板的"逐点讲透"是常见组合
...
| **多概念对比（D 模板）** | **`03-mysql/04-log.md` 第一章** | **一句话定位 + 多维度对比 + 协作时序 + 缺一不可** ⭐ |
