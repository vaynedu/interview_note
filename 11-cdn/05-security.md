...
### 3.7 WAF vs DDoS 高防 IP（核心区分）★

> **最容易混淆的两个概念，名字里都有"防"，但完全不同维度。**

#### 一句话区分

| 工具 | 防什么 | 看什么 |
| --- | --- | --- |
| **WAF** | 防"内容坏" | 看请求**长什么样**（SQL 注入 / XSS 等业务攻击）|
| **DDoS 高防 IP** | 防"量太大" | 看流量**有多少**（Tbps 级洪水）|

→ **互补，不可替代**：WAF 拦不住流量洪水，高防 IP 拦不住单个 SQL 注入。

#### 用真实攻击对比

**攻击 A：SQL 注入（1 个请求就能搞垮）**

```http
GET /api/user?id=1' UNION SELECT password FROM users--

特征:
  - 流量小（1 个请求）
  - 看起来"正常"（HTTP 格式合法、IP 正常）
  - 但内容里藏着 SQL 注入 payload
...
**攻击 B：HTTP Flood / CC 攻击（100w 个"正常"请求）**

```http
GET /api/search?q=keyword
GET /api/search?q=keyword  ← 同样的请求被 100w 个肉鸡发
GET /api/search?q=keyword
... × 100w

特征:
  - 每个请求都"正常"（合法 HTTP，无注入）
  - 但 100w QPS 海量袭击
  - 目标: 打爆服务器 CPU / 带宽 / 数据库

谁拦得住:
  ✗ WAF（每个请求都正常，看不出问题）
  ✓ 高防 IP（识别速率异常 + IP 信誉 + 行为）
```

**攻击 C：SYN Flood / UDP Flood（L3/L4 网络层）**

```
1Tbps 的 SYN 包 / UDP 包打过来
特征:
  - 极大带宽攻击
  - 根本到不了应用层（在 TCP 握手就死）
...
| 工具 | 主战场 | 解决问题 | 类比 |
| --- | --- | --- | --- |
| **CDN** | L7 加速 + 轻量 DDoS / WAF | 加速 + 基础防护 | 全国分店（流量分散）|
| **WAF** | L7 内容检测 | 业务漏洞攻击 | 安检员（检查行李内容）|
| **DDoS 高防 IP** | L3/L4 + 大流量 L7 | 海量流量攻击 | 防洪堤（拦超大水量）|
...
WAF 接入:
  example.com CNAME → waf.example.com → 源站
  或源站部署 ModSecurity（Nginx 模块）

高防 IP 接入:
  example.com A 记录 → 高防 IP（203.x.x.x）
  高防转发到源站（带 X-Forwarded-For 真实 IP）
...
| 业务规模 | 推荐组合 | 月成本（参考）|
| --- | --- | --- |
| 中小公司 / 普通业务 | **CDN**（自带 WAF + 基础 DDoS）| 几千 / 月 |
| 中型互联网 | **CDN + 云 WAF** | 几万 / 月 |
| 金融 / 高价值业务 | **CDN + WAF + DDoS 高防 IP** | 几十万 / 月 |
| 受攻击时（紧急）| **临时开高防 IP**（按峰值计费）| 几万 / 几小时 |
| BAT 级 | 以上 + **自建清洗中心** | 千万级 / 年 |

#### 一句话总结

> **WAF ≠ DDoS 高防 IP，互补不替代**：
>
> - **WAF** = "内容层警察"——看每个请求的**内容**，拦 SQL 注入 / XSS / 爬虫等"少量但危险"的攻击；**抓个例的精准防御**
> - **DDoS 高防 IP** = "流量层防洪堤"——看流量**总量和模式**，扛 SYN Flood / HTTP Flood 等"海量"的攻击；**抗洪水的容量防御**
>
> **关键认知**：
> - WAF 看到 100w 个"看起来正常"的请求，每个都放行 → 源站被打爆
> - 高防 IP 看到 1 个 SQL 注入请求，量小不算异常 → 放行到源站 → 数据库被脱
> - 必须**两个都有**才完整（CDN 自带"轻量版"两者，超大场景才需要单独高防 IP）
>
> **类比**：WAF 是机场安检（检查行李是否有违禁品），高防 IP 是防洪堤（扛得住大水），CDN 是高速公路（让正常流量通畅）——三个各司其职。
