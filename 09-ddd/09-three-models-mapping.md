...
> 资深视角的"5 段式"在下一节,这一节先用**不写一行代码**的方式把概念讲透——如果你做了多年开发还是迷糊,大概率是**一直在看技术架构和数据模型,反推不出业务模型,需要往上看两层**。

### 1. 业务模型的一句话定义

> **业务模型 = 不写一行代码,用日常语言把一件事讲清楚——谁参与、有什么东西、做什么动作、遵守什么规则**。

注意三个否定(业务模型**不是**什么):

- ❌ **不是数据库设计**(没有表、字段、索引)
- ❌ **不是接口文档**(没有 URL、参数、返回值)
- ❌ **不是技术架构**(没有 Redis、MySQL、K8s)

业务模型是**业务侧的语言**——连产品经理、运营、客服都能听懂。

### 2. 四个容易混淆的概念对比

| 概念 | 描述什么 | 谁能看懂 | 例子 |
| --- | --- | --- | --- |
| **业务模型** | 系统在干什么事 | 老板、运营、PM、客服 | "用户领券、下单时用券抵扣,过期作废" |
| **领域模型** | 业务模型的对象化抽象 | 程序员 + PM | `Coupon`、`Order`、`User`、`CouponUsage` 几个实体 |
| **数据模型** | 数据怎么存 | 程序员 + DBA | `coupon` 表、`user_coupon` 表、字段、索引 |
| **技术架构** | 代码怎么部署 | 程序员 + SRE | Redis 扣减 + MySQL 持久化 + K8s 多副本 |

**4 层关系(从抽象到具体)**:

```
业务模型     ← 最抽象,给业务人员看(老板/PM/运营/客服)
   ↓ 程序员翻译
领域模型     ← 程序员把业务对象化(Coupon/Order 类)
   ↓ 持久化设计
数据模型     ← 把领域对象落到存储(表/索引/分片)
   ↓ 部署运行
技术架构     ← 把数据模型跑起来(Redis/K8s/网关)
```

> **迷茫的根因**:大多数程序员一直在最底两层(数据模型 + 技术架构)打转,**反推不出业务模型,因为业务模型在上面两层**——必须**主动往上看两层**才看得到。

### 3. 业务模型由 4 个东西组成(WWDR 法则)

任何业务模型只需回答 4 个问题:

| 要素 | 回答什么 | 类比 | 优惠券例子 |
| --- | --- | --- | --- |
| **角色(Who)** | 谁在用这个系统 | 话剧的**人物表** | 用户 / 商家 / 运营 / 财务 |
| **实体(What)** | 系统里有哪些"东西" | 话剧的**道具** | 券模板 / 用户券 / 订单 / 核销记录 |
| **行为(Do)** | 角色对实体做什么动作 | 话剧的**情节** | 用户领券 / 用户下单核销 / 运营发券 / 系统过期作废 |
| **规则(Rule)** | 行为受什么约束 | 剧本的**设定** | 一人限领 1 张 / 满 100 减 20 / 7 天过期 / 不可叠加 |
...
【行为】
  - 运营创建券模板 → 系统按规则生成可领券池
  - 用户领券 → 系统校验资格 + 库存 → 写入用户券包
  - 用户下单 → 系统计算最优券 + 抵扣 → 核销标记
  - 用户退单 → 系统判断是否返券
  - 定时任务 → 扫描过期 → 自动作废
...
→ **整个过程不出现 Redis、MySQL、Kafka、Lua、HTTP**——这就是合格的业务模型。

> **一句话总结**:业务模型 = **WWDR 四要素(Who 角色 + What 实体 + Do 行为 + Rule 规则)**,**不出现任何技术词汇**,产物是业务专家/PM/运营/客服都能读懂的描述;**程序员迷茫的根因是一直在看技术架构和数据模型,要主动往上看两层**才能看到业务模型。
