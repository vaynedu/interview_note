...
### C.0 核心信念与面试表达模板(先背这个)

**三句话信念**(资深面试可直接背):

1. **分桶是为 QPS 服务的,不是为库存大小服务的**——单 SKU 库存 10w 件不用分桶,QPS 10w 才用
2. **展示和扣减是两件事**——展示走聚合 + 缓存,扣减走分桶 + Lua
3. **动态决策,不要全量分桶**——商品中心标记热点 + 大促前预热

**三层职责分工(架构精髓)**:

```text
分桶          ─→ 实时扣减(写主路径,高 QPS,精确到原子操作)
异步汇总      ─→ 展示剩余库存(读主路径,允许秒级延迟)
DB/订单流水   ─→ 最终一致(对账兜底,T+1 修复)
```

| 层 | 职责 | 一致性 | 性能 |
| --- | --- | --- | --- |
| Redis 分桶 | 扣减/释放/实扣 | 强一致(单 bucket) | 微秒级,10w/桶 |
| 聚合 view | 展示「还剩 N 件」 | 秒级延迟可接受 | 缓存即可 |
| MySQL 库存表 | 真相之源 | 最终一致 | 异步对账 |

**展示库存的智能梯度策略**(关键资深点):

```text
剩余 > 阈值(如 100):  显示「有货」,不查实时数据(用缓存,秒级过期)
剩余接近阈值:         收紧检查,聚合所有 bucket 真实剩余
剩余 ≈ 0:            走最严格逻辑,串行扫 N 个 bucket(此时 QPS 已降)
```

**为什么这样做**:大促 99% 时间库存充裕,根本不需要精确"还剩 N 件";只有最后 1%(卖空前夜)才需要精确——但此时 QPS 已降下来,有性能预算去精确扫描。**性能-精度的动态权衡**。

**分桶数动态决策**:

| SKU 类型 | 桶数 | 判断依据 |
| --- | --- | --- |
| 普通 SKU | 1 桶(不分桶) | 日常 QPS < 1w |
| 热点 SKU | 16-32 桶 | 大促爆款,QPS 5-50w |
| 超级热点 | 64-128 桶 | iPhone 首发级,QPS 100w+ |

**为什么不全量分桶**:
- key 数量爆炸:1000w SKU × 32 桶 = 3.2 亿 key,内存 + 管理灾难
- 90% SKU 是冷门长尾,达不到 1w QPS
- 分桶 = 复杂度,只为有需要的 SKU 付出

**面试现场表达模板**:

> 热点 SKU 假售罄本质是 Redis Cluster 单 key 单节点的硬限制。我们的解决思路分三步:
>
> **第一步**:商品中心标记热点 SKU——只对识别出的爆款分桶,日常 SKU 保持单 key,避免无意义的复杂度。
>
> **第二步**:分桶的 key 用 `{sku_id_bN}` hash tag 锁分片,把单 SKU 的扣减能力从单节点扩展到 N 节点。bucket 数动态决定:普通 1 桶、热点 16-32 桶、超级热点 64+ 桶。
>
> **第三步**:把"实时扣减"和"展示库存"职责分开——分桶负责强一致扣减,异步聚合负责展示,DB 流水负责最终一致。展示库存有梯度策略:库存充裕时显示"有货"不查精确数,接近售罄时才聚合所有 bucket。
>
> 最后还有兜底:Go 端跨 bucket 顺次扫描防假售罄,大促结束后定时合并零散 bucket 回中心,T+1 对账修复任何 Redis/MySQL 不一致。业界主流是这个套路——阿里淘宝、抖音电商、京东都基本如此。秒杀级别(小米、12306)会更激进,走本地缓存预扣 + 中心校验。
...
## 附录 D：同商品两次下单是否算重复(四层防重)

> 「用户对同一商品两次下单,如何判断是不是重复下单?」这道题极易答歪——"重复"有**两种本质完全不同的含义**,粗暴的"同 SKU 同用户拒绝第二单"会误杀真实需求。本附录系统讲清「分层识别 + 友好引导 + 兜底强制」的组合方案。
...
| 类型 | 用户意图 | 系统应该 |
| --- | --- | --- |
| **A. 技术层重复**(误触) | 只想下 1 单,网络抖动/按钮连点发了 2 次 | **必须阻止**,只生成 1 个订单 |
| **B. 业务层重复**(故意) | 真的想下 2 单(自用 + 送礼) | **允许**,但可能触发限购规则 |

**核心信念**:不能简单粗暴"同 SKU 同用户拒绝第二单"——会误杀真实需求。要用**多层机制分别识别**:
- 技术层 request_id 防误触
- 业务层未付订单引导
- 运营层限购规则
- 风控层防灰产
- DB 层 UNIQUE 兜底(仅秒杀场景)

### D.1 第一层:request_id 幂等(技术层)

```text
前端:
  用户点击「立即下单」按钮
  → 生成 UUID 作为 request_id(本地唯一,只此一次)
  → 按钮置灰,防止重复点击

后端:
  SETNX idem:{user_id}:{request_id} = "PENDING" EX 10
  - 成功 → 新请求,继续处理
  - 失败 → 同一次点击的重试,读 cached order_id 返回

特征:
  - 仅防 5-10 秒内的「同次点击重试」
  - request_id 由前端生成,关浏览器/重开页面会生成新的
  - 用户重新点击「下单」 = 新的 request_id = 新订单(不算重复)
```

**这一层解决的是「网络抖动 / 用户狂点」**,不是「用户故意下两单」。详见 [附录 A](#附录-aredis-lua-库存扣减完整实现) + 5.1 链路第 2 步。

### D.2 第二层:未支付订单检查(业务层最直观)
...
```text
count > 0 → 弹窗:「您有未支付订单 #xxx,是否先去支付?」
  - 用户选「继续支付」→ 跳转旧订单
  - 用户选「重新下单」→ CANCEL 旧订单(释放库存) + 创建新订单
count = 0 → 正常下单
```

**这一层解决的是「用户忘记之前下过单」**——给出明确选择,而不是默默拒绝。
...
Redis 计数:
  INCR limit:user:{user_id}:sku:{sku_id}:day:{date}
  EXPIRE 86400(首次)

判断:
  if count > 5: 返回「超过单日限购」
...
**资深点**:限购规则**必须是运营可配置的**,不写死代码——参考 [14b-coupon-system-4s.md](14b-coupon-system-4s.md) 的规则引擎设计。
...
**这一层解决的是「灰产 / 羊毛党」**——大促必须有,日常按需开启。

### D.5 第五层:DB UNIQUE 兜底(最严格场景)

如果业务要求「同 SKU 同用户 1 单不能多」,DB 层加 UNIQUE 强制兜底:

```sql
-- 大促爆款专用:UNIQUE 强制单用户单 SKU 单单
CREATE TABLE flash_orders (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    order_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    sku_id BIGINT NOT NULL,
    campaign_id BIGINT NOT NULL,  -- 活动 ID
    ...
    UNIQUE KEY uk_user_sku_campaign (user_id, sku_id, campaign_id)
);

-- 第二单插入直接 DUPLICATE_KEY 报错,DB 层兜底
```

**适用场景**:秒杀、限购 1 件的大促活动——参考 [03-seckill-system.md](03-seckill-system.md) 和 [14b-coupon-system-4s.md](14b-coupon-system-4s.md) 的双 UNIQUE 思路。

**不要日常用**:UNIQUE 会让用户**任何场景都无法重买**,严重影响正常需求。

### D.6 完整决策流程图

```mermaid
flowchart TD
    A[用户点击下单] --> B{L1 request_id<br/>幂等检查}
    B -->|命中| C[返回缓存 order_id<br/>技术层重复]
    B -->|未命中| D{L4 风控扫描}
    D -->|高危| E[强制验证码 / 拒绝]
    D -->|通过| F{L2 未支付订单检查}
    F -->|有未付订单| G[弹窗提示<br/>跳旧订单 / 取消重下]
    F -->|无| H{L3 限购规则检查}
    H -->|超限| I[返回「超过限购」]
    H -->|通过| J{L5 DB UNIQUE<br/>仅秒杀活动}
    J -->|冲突| K[返回「已购买」]
    J -->|通过| L[正常落库]
```
...
| 场景 | 表现 | 识别信号 | 处理 |
| --- | --- | --- | --- |
| **网络抖动重试** | 同 request_id | L1 SETNX | 返回缓存 order_id |
| **用户连点按钮** | 前端按钮防抖失败 | L1 SETNX | 返回缓存 order_id |
| **用户忘记付款** | 不同 request_id,有 WAIT_PAY 订单 | L2 DB 查询 | 弹窗引导 |
| **用户真想下 2 单** | 不同 request_id,旧单已处理 | L3 限购检查 | 正常处理 / 超限拒绝 |
| **黄牛 / 灰产刷单** | 高频 / 异常 IP / 设备 | L4 风控 | 验证码 / 拉黑 |
| **秒杀活动重复** | 限购 1 件 / 用户 | L5 UNIQUE | DB 兜底拒绝 |

### D.8 普通答法 vs 资深答法

| 普通答法 | 资深答法 |
| --- | --- |
| 用 request_id 防重 | **区分技术 vs 业务两种重复,分层处理** |
| 拒绝同用户多次下单 | **大多数场景不应该拒绝,弹窗引导让用户选** |
| MySQL UNIQUE 兜底 | **UNIQUE 只用在限购 1 件的活动,不能日常用** |
| Redis 计数器限购 | **限购规则应该是运营可配置的,不写死代码** |
| 没想到风控 | **大促必须有风控,高频/异常触发验证码** |
| 一层防重 | **五层组合,各司其职** |

### D.9 面试现场表达模板

> 这道题关键是先**澄清「重复下单」是什么含义**——是网络抖动的技术重复,还是用户真想下两单的业务重复?这两者要分别处理。
>
> **第一层 request_id 幂等**(技术层):前端生成 UUID,后端 SETNX 5-10 秒,防的是同一次点击的网络重试。这只解决「误触」,不解决「故意」。
>
> **第二层未支付订单检查**(业务层):用户点下单时,后端先查有没有同 SKU 的 WAIT_PAY 订单,有则弹窗让用户选——继续支付旧订单,还是取消重下。这是大多数电商的友好做法。
>
> **第三层限购规则**(防黄牛):Redis 计数器,同 SKU 同用户单日 N 单,大促爆款可以收紧到 1 单。这一层是运营可配置的。
>
> **第四层风控**(防灰产):高频下单 / 异常 IP / 设备指纹触发,走人机验证或异步审核。
>
> **最严格的场景如秒杀**,直接用 MySQL 的 UNIQUE(user_id, sku_id, campaign_id) 兜底,DB 层强制单用户单 SKU 一单。
>
> 这套四层组合既不误杀真实需求(用户真的想下 2 单是合法的),又能拦住技术误触和恶意刷单。业界淘宝、抖音电商、京东大致都是这套思路。

### D.10 资深追问预案

| 追问 | 回答 |
| --- | --- |
| request_id 谁生成? | **前端**生成(UUID),不能服务端生成——服务端生成无法做「按钮重复点击」的幂等 |
| request_id TTL 多长? | 5-10 秒,只防同次点击重试。长 TTL 会卡死用户合法重试 |
| 用户清掉缓存重下算重复吗? | **不算**——新 request_id 是新意图,走完整业务规则即可 |
| 弹窗"是否取消旧单"用户取消了怎么办? | 走 [5.3 超时取消链路](#53-超时取消链路delay-queue) 的释放逻辑,清掉 reserve 库存 |
| 限购规则怎么动态生效? | 规则引擎 JSON 配置 + 本地 LRU + Redis 二级缓存,详见 [14b §3.6](14b-coupon-system-4s.md) |
| 多设备同账号下单算重复吗? | 不算——账号是唯一标识,设备只是入口 |
| UNIQUE 索引在分库分表后还有效吗? | 有效,因为按 user_id 分片,user_id+sku_id+campaign_id 必在同分片 |
| 风控误伤了正常用户怎么办? | 走人机验证(验证码 / 滑动),通过即放行;过度严格会损失转化 |
| 大促和日常的防重策略一样吗? | **不一样**——日常 L1+L2 即可,大促必须叠加 L3+L4,秒杀加 L5 |
