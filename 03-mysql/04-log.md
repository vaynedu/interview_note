...
> **一句话总结**:undo log **双重身份**——事务回滚 + MVCC 历史版本链,**长事务是头号杀手**(history list length 涨爆 ibdata),记口诀"undo 不只回滚,还撑 MVCC"。
...
> **一句话总结**:redo log **物理页级日志 + WAL + 循环写**,把随机写变顺序写,核心解决"提交后宕机数据不丢",**InnoDB 引擎层独占**(binlog 替代不了)。
...
> **一句话总结**:binlog **Server 层逻辑日志 + 追加写**,核心用于**主从复制 / PITR 时间点恢复 / 数据订阅(canal)**,生产首选 Row 格式(Statement 有非确定性 SQL 风险)。
...
> **一句话总结**:2PC = **redo prepare → binlog → redo commit 三步**,核心保证 **InnoDB 崩溃恢复结果和 binlog 一致**(否则主从永久不一致),崩溃判断口诀"**prepare 但 binlog 不全 → 回滚,prepare 且 binlog 完整 → 提交**"。
...
> **一句话总结**:redo vs binlog **四维差异**——层次(引擎 vs Server)、内容(物理 vs 逻辑)、写法(循环 vs 追加)、用途(崩溃恢复 vs 复制/PITR),**互不可替代**。
...
> **一句话总结**:WAL = **先写日志(顺序)再刷数据页(随机)**,核心是"日志顺序写比数据页随机写快几个数量级",这是数据库提交事务为啥能扛万级 TPS 的根本。
...
> **一句话总结**:binlog **不能替代 redo**——binlog 是 Server 层逻辑日志,**不知道 InnoDB 页的物理细节**,做不了崩溃恢复中的页级幂等重放。
...
> **一句话总结**:redo **不能替代 binlog**——InnoDB 内部日志不跨引擎、循环写不留历史,**主从复制和 PITR 必须靠 binlog**。
...
> **一句话总结**:binlog 格式选 **Row**(生产首选)——日志大但**复制一致 + 数据订阅(canal)友好 + 闪回准确**,Statement 有非确定性 SQL 风险(`NOW()`/`UUID()`/触发器),Mixed 行为不可预测。
...
> **一句话总结**:提交后宕机丢不丢数据看 **`innodb_flush_log_at_trx_commit` + `sync_binlog`**——双 1(`=1` + `=1`)零丢失但性能最差,生产常用双 1(金融) / `2/1000`(性能优先) 折中,**刷盘越严丢数据越少 → 性能越差**。
...
> **一句话总结**:误删恢复 = **全量备份 + binlog 回放(跳过误操作)**,核心**临时实例校验后回灌主库**(绝不在主库操作),前提是**备份完整+ binlog 连续+定期演练**(没演练过的备份等于没有)。
...
> **一句话总结**:大事务 = **主从延迟元凶**——binlog 一次提交几 GB,从库**串行重放卡分钟到小时级**,期间从库读全部延迟,治理"**拆小批次 + 控制行数 + 事务内禁外部调用**"。
...
> **一句话总结**:三类日志七大坑可归三类——**概念混淆**(以为 binlog 能崩溃恢复 / redo 能复制)、**长事务连锁**(undo 涨爆 + 主从延迟)、**运维侥幸**(备份不演练 / 降刷盘没评估丢数据风险)。
...
> **一句话核心(全篇精炼)**:MySQL 三类日志 = **undo(回滚+MVCC,InnoDB)+ redo(崩溃恢复+WAL,InnoDB 循环写)+ binlog(复制+PITR,Server 追加写)**,2PC(prepare→binlog→commit) 保证 redo 和 binlog 一致,**互不可替代**;口诀"**A 靠 undo、D 靠 redo、复制靠 binlog、长事务必杀**"。
