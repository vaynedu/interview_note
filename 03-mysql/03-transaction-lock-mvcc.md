...
> **一句话总结**:ACID 各有依赖——**A 靠 undo(回滚)、I 靠锁+MVCC、D 靠 redo(刷盘)**,**C 不是单点保证的**,它是约束+事务+业务三方协作的结果,答 ACID 别说成"四个并列特性",要说出对应机制。
...
> **一句话总结**:四级隔离防三现象,关键区分是**目标对象不同**——脏读防"读到未提交"、不可重复读防"单行被改"、幻读防"行集合被插/删",**幻读和不可重复读最大区别是粒度**(单行 vs 行集合,UPDATE vs INSERT/DELETE)。
...
> **一句话总结**:**隔离级别 = 锁的组合,不是玄学**——RU 只有写锁、RC 加短读锁、RR 读锁持到事务结束、SR 再加范围锁,**每升一级多加一种锁/持更久**,一致性↑并发↓,理解了这个就不需要背隔离级别表。
...
> **一句话总结**:InnoDB 的 RR 突破了 SQL 标准——**快照读靠 MVCC(Read View 锁定一致版本),当前读靠 Next-Key Lock(行锁+间隙锁)**,两条路都防住幻读,这是面试"InnoDB 默认 RR 还防住幻读"的根本答案。
...
> **一句话总结**:**MySQL RR 默认是历史包袱**(早期 statement binlog 要求),业界普遍 RC——**大厂内部 MySQL 也改成 RC**(间隙锁死锁多 + row binlog 已普及),知道这个能在面试聊出"为什么默认 RR 但不推荐 RR"。
...
> **一句话总结**:MVCC 三件套 = **隐藏字段 + undo log 版本链 + Read View**,RC/RR 唯一差异是 **Read View 创建时机**(每次 SELECT vs 事务首次 SELECT 后复用)——记住这一句就能秒杀"为啥 RR 看到一致快照"。
...
> **一句话总结**:**普通 SELECT = 快照读走 MVCC 不加锁,FOR UPDATE/UPDATE/DELETE = 当前读读最新加锁**——这是 MVCC"读不阻塞写"的边界,讲 RR 防幻读必须先说清楚这两条路。
...
> **一句话总结**:InnoDB 锁分**模式**(共享 S / 排他 X)+ **粒度**(记录锁 / 间隙锁 / 临键锁 / 意向锁),**临键锁 = 记录锁 + 左开右闭间隙锁是默认形式**;**死亡陷阱**是"行锁加在索引上"——SQL 没命中索引会退化成扫表加锁,锁范围爆炸,这是 RR 死锁的最大根因。
...
> **一句话总结**:MVCC 解决**读写并发**(读不阻塞写、写不阻塞读),代价是**空间换并发 + 长事务撑爆 undo**——**写写冲突、当前读、热点行**这三种它救不了。
...
> **一句话总结**:答 MySQL 默认 RR **不能只说"可重复读"**——要展开"普通 select 走 MVCC 快照读 + 当前读走 Next-Key Lock,两路都防幻读",这才是资深答法。
...
> **一句话总结**:间隙锁的存在**只为防当前读的幻读**——锁住索引区间不让别人 INSERT,代价是**并发掉得厉害**(大厂改 RC 的核心动机)+ 死锁多发(两个事务锁不同间隙互等)。
...
> **一句话总结**:死锁根因是**加锁顺序不一致**——InnoDB 能自动检测并回滚一个事务,但业务必须捕获 1213 错误**指数退避重试**,预防三件套**固定加锁顺序+缩短事务+SQL 走索引**(避免锁扩大)。
...
> **一句话总结**:扣库存防超卖**核心是 `where stock > 0` 把判断下推到 DB 层 + 排他锁原子化**,绝不能"先查再改";高并发热点 SKU 必须**分桶/预扣/队列削峰**,单行 update 撑不住几万 QPS。
...
> **一句话总结**:转账事务核心**5 条铁律**——一个事务内完成、**按 ID 升序加锁防死锁**、金额用整数分(避浮点)、事务内禁外部调用(RPC/HTTP)、超时必须**幂等+对账**兜底。
...
> **一句话总结**:长事务是 MySQL **头号杀手**——锁不放(阻塞)、Read View 不放(undo 涨爆 ibdata)、回滚成本爆炸、主从延迟、连接占满,治理口诀**"事务内只放 SQL,RPC/HTTP/大批量全部拆出去"**,监控 `information_schema.innodb_trx`。
...
> **一句话总结**:事务/锁/MVCC 七大坑可归三类——**概念混淆**(快照读 vs 当前读、RR 完全无幻读)、**SQL 写法毁锁**(无索引扫表锁、先查再改)、**长事务连锁**(undo 膨胀+主从延迟+锁不放)。
...
> **一句话核心(全篇精炼)**:事务/锁/MVCC = **ACID 四依赖(A 靠 undo、I 靠锁+MVCC、D 靠 redo、C 三方协作)+ 隔离级别=锁组合(每升一级多加锁/持更久)+ MVCC 三件套(隐藏字段+undo 链+Read View,RC/RR 差异在创建时机)+ InnoDB RR 防幻读两路(快照读 MVCC、当前读 Next-Key)+ 长事务是头号杀手**,口诀"**隔离=锁组合、MVCC=空间换并发、长事务必杀**"。
